...
interface VisitFormProps {
  ...
  /** Type can only be chosen while creating; it's fixed afterwards. */
...
export default function VisitForm({ mode, initial, typeEditable, onSubmit, onCancel }: VisitFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (nextErrors.name || nextErrors.notes) return;

    setPending(true);
    const ok = await onSubmit({ name: name.trim(), type, notes: notes.trim(), visitedAt });
    ...
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      ...
      <div>
        <label htmlFor="visit-date" className="block text-sm font-medium">
          Visited on <span className="font-normal text-slate-400">(optional)</span>
        </label>
        <input
          id="visit-date"
          type="date"
